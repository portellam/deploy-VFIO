    ...
    for (( int_indexVGA=0; int_indexVGA<${#arr_VGABusID[@]}; int_indexVGA++ )); do
        ...
        if [[ $str_input2 != "Y" ]]; then
            ...
            for (( int_indexPCI=0; int_indexPCI<${#arr_PCIBusID[@]}; int_indexPCI++ )); do
                ...
                if [[ $str_thisPCIBusID == "01:00.0" ]]; then
                    bool_parsePCIifExternal=true
                fi
                #
                ...
                if [[ $str_thisVGABusID != $str_thisPCIBusID && $str_thisVGADriver == $str_thisPCIDriver ]]; then
                    str_thisPCIDriver=""
                fi
                #
                ...
                if [[ ${str_thisVGABusID:0:5} == ${str_thisPCIBusID:0:5} || $str_thisPCIDriver == $str_thisVGAChildDriver || $str_PCIDriver_list == *"$str_thisPCIDriver"* ]]; then
                    str_thisPCIDriver=""
                fi
                #
                ...
                if [[ $bool_parsePCIifExternal == true && ! -z $str_thisPCIDriver ]]; then
                    str_PCIDriver_list+="$str_thisPCIDriver,"
                fi
                #
            ...
            if [[ ${str_PCIDriver_list: -1} == "," ]]; then
                str_PCIDriver_list=${str_PCIDriver_list::-1}
            fi

            if [[ ${str_listPCIHWID: -1} == "," ]]; then
                str_listPCIHWID=${str_listPCIHWID::-1}
            fi
            #
  
            # GRUB command line #
            str_GRUB_CMDLINE="acpi=force apm=power_off iommu=1,pt amd_iommu=on intel_iommu=on rd.driver.pre=vfio-pci pcie_aspm=off kvm.ignore_msrs=1 $str_GRUB_CMDLINE_Hugepages modprobe.blacklist=$str_PCIDriver_list vfio_pci.ids=$str_listPCIHWID"
...
"
            ...
            for (( int_indexPCI=0; int_indexPCI<${#arr_PCIBusID[@]}; int_indexPCI++ )); do
                ...
                if [[ $str_PCIDriver_list == *"$str_thisPCIDriver"* ]]; then str_thisPCIDriver=""; fi
                #

                # if no PCI driver match found (if string is not empty), add to list #
                if [[ $bool_parsePCIifExternal == true && ! -z $str_thisPCIDriver ]]; then str_PCIDriver_list+="$str_thisPCIDriver,"; fi
                #
            ...
            # remove last separator #
            if [[ ${str_PCIDriver_list: -1} == "," ]]; then str_PCIDriver_list=${str_PCIDriver_list::-1}; fi
...
function StaticSetup {
    ...
    while [[ $bool_isVFIOsetup == false ]]; do
        ...
        ## parse list of PCI devices by IOMMU ID ##
        # parse list of group IDs #

        ## add check that ignores onboard PCI if no external PCI is parsed
