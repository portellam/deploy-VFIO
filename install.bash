...
# install required packages #
    ...
    {
        echo -e "Linux distribution found: "$(lsb_release -is)"\nChecking for updates..."

        if [[ $(command -v apt) != "/usr/bin/apt" ]]; then
            apt update && apt full-upgrade -y && apt install -y git libvirt0 qemu virt-manager zram-tools

        else
            echo -e "WARNING: Check for updates failed. Linux distribution is not compatible with '${0}'."
        fi

        # echo -e "Linux distribution found: `lsb_release -i -s`\nChecking for updates...\n"

        # # Arch family
        # if [[ $(command -v pacman) != "/usr/bin/pacman" ]]; then
        #     pacman -Syu && pacman -Sy edk2-ovmf lgit ibvirt qemu-desktop virt-manager zramswap
        #     # NOTE: find same packages from Debian

        # # Debian family
        # elif [[ $(command -v apt) != "/usr/bin/apt" ]]; then
        #     apt update && apt full-upgrade -y && apt install -y git libvirt0 qemu virt-manager zram-tools

        # # Fedora family
        # elif [[ $(command -v dnf) != "/usr/bin/dnf" ]]; then
        #     dnf check-update && dnf upgrade -y && dnf install -y @virtualization git zram-generator zram-generator-defaults

        # else
        #     echo -e "Checking for updates... Failed.\nLinux distibution is not recognized for or compatible with 'deploy-VFIO-setup'.\nExiting."
        #     exit 1
        # fi
