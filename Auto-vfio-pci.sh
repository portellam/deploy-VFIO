...
function Xorg {
    ...
    # set file #
    str_file1="/usr/sbin/Xorg-vfio-pci.sh"
    str_file2= "/etc/systemd/system/Xorg-vfio-pci.service"
    #

    # parameters #
    declare -a arr_XorgScript=(`#!/bin/bash

# Description: Generate Xorg for first available VGA device.
...
while read -r str_line; do
    ...
    if [[ $str_line == *"VGA"* && $str_line != *"Subsystem: "* && $str_line != *"Kernel driver in use: "* && $str_line != *"Kernel modules: "* ]]; then     # line includes PCI bus ID
        ...
        ((int_index++))                     # increment only if Bus ID is found
        str_thisVGABusID=(${str_line:0:7})  # VGA Bus ID
        bool_parseVGA=true
...
if [[ -z $str_VGABusID || -z $str_VGADriver ]]; then bool_invalidVGA=true;
...
# Xorg conf generated by Xorg-vfio-pci #
Section "Device"
Identifier     "Device0"
...
EndSection')
    ...
    declare -a arr_XorgConf=('
# Xorg conf generated by Xorg-vfio-pci #
# WARNING: no VGA device found! Execute 'lscpi -k', and rewrite this file for available VGA devices.
#Section "Device"
#Identifier     "Device0"
#Driver         "driver_name"
#BusID          "PCI:x:x:x"
...
# restart service #
str_input=$"echo $1 | tr '[:lower:]' '[:upper:]'")
if [[ $str_input == "DM" ]]; then
...
WantedBy=multi-user.target")
    ...
    # write to file #
    for str_line in $arr_XorgScript; do echo $str_line >> $str_file1; done
    for str_line in $arr_XorgService; do echo $str_line >> $str_file2; done
    #
